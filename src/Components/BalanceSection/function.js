...
export const initialStore = (name) => {
  const storeIncome = [
    { name: 'Зарплата', id: Date.now() + 1, value: [] },
    { name: 'Премии', id: Date.now() + 2, value: [] },
    { name: 'Подарки', id: Date.now() + 3, value: [] }
  ];

  const storeExpenses = [
    { name: 'Продукты', id: Date.now() + 4, value: [] },
  ...
  try {
    ...
    return store || (name === 'Income' ? storeIncome : storeExpenses);
  } catch (e) {
...
export const includes = (store, str) => store.map((item) => item.name.toLowerCase()).includes(str.trim().toLowerCase());

export const total = (store) => store.reduce((res, i) => (res += i.value.reduce((res, i) => (res += i.value), 0)), 0);
